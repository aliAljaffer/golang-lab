...
A common mistake (especially for Java/C# folks) is to define a giant `INotifier` interface in the package that _implements_ it, then have the consumer depend on it. That makes the interface a contract the producer has to maintain, even if a consumer only needs one method.
