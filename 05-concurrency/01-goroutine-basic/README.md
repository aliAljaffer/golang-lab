...
- Capture-by-reference gotcha (pre-Go 1.22): `for i := 0; i < 5; i++ { go func() { fmt.Println(i) }() }` prints `5 5 5 5 5` because the closure captures the _loop variable_. Pass `i` as an argument, or in Go 1.22+ rely on per-iteration scoping.
