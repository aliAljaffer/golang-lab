...
- `ctx.Err()` tells you _why_: `context.Canceled`, `context.DeadlineExceeded`, or nil if still live.
...
- Cancellation is cooperative. The runtime does not interrupt your goroutine. _You_ must select on `ctx.Done()` and return.
