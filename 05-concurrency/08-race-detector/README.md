...
Two goroutines access the same memory location, **at least one of them writes**, and there is no synchronization (channel send/recv, mutex, atomic) ordering them. The result is undefined: torn reads, lost writes, "impossible" branches. Programs *may appear* to work and then fail in production at the worst time.
...
## Common races the detector does *not* catch
