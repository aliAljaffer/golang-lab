// gcs-log-shipper — tail local logs, batch+gzip them, upload to GCS.
//
// The testable surface lives in tail.go / batch.go / upload.go / run.go.
// This file is just the cobra wiring + GCS client construction and is not
...
func newRootCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "gcs-log-shipper",
		Short: "Tail local logs, batch+gzip, upload to GCS",
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			client, err := storage.NewClient(cmd.Context())
			if err != nil {
			...
			uploader := &GCSUploader{
				Inner:       &gcsClientAdapter{Client: client},
				Bucket:      opts.Bucket,
	...
	cmd.Flags().IntVar(&opts.MaxRetries, "max-retries", 3, "retry count for transient GCS errors")
	return cmd
}

// ctxSleep is a ctx-cancellable sleep used by GCSUploader for backoff. Lives
