...
// TestBatcher_CRCIsCastagnoli is the load-bearing GCS-specific regression.
// If finalize() builds its CRC table with crc32.IEEE (the default) instead
// of crc32.Castagnoli, the test will fail because the two polynomials
// produce different hashes for any non-trivial input.
func TestBatcher_CRCIsCastagnoli(t *testing.T) {
	now := fixedNow
	...
	castagnoli := crc32.Checksum(batch.Body, crc32.MakeTable(crc32.Castagnoli))
	ieee := crc32.ChecksumIEEE(batch.Body)

	if batch.CRC32C != castagnoli {
		t.Errorf("Batch.CRC32C = %#x; want Castagnoli %#x (GCS uses Castagnoli, not IEEE)", batch.CRC32C, castagnoli)
	}
	if batch.CRC32C == ieee && castagnoli != ieee {
		t.Errorf("Batch.CRC32C matches IEEE %#x — that is the wrong polynomial for GCS", ieee)
	}
}

// ---- GCSUploader (retry/backoff) -----------------------------------------
...
func TestGCSUploader_TransientThenSuccess(t *testing.T) {
	transient := status.Error(codes.Unavailable, "boom")
...
func TestGCSUploader_PermanentNoRetry(t *testing.T) {
	permanent := status.Error(codes.PermissionDenied, "no access")
	pp := &programmedPutter{errs: []error{permanent}}
	u := &GCSUploader{Inner: pp, Bucket: "b", MaxRetries: 5, BaseBackoff: time.Millisecond, Now: time.Now, Sleep: fastSleep}
...
func TestGCSUploader_BackoffBounded(t *testing.T) {
	transient := status.Error(codes.Unavailable, "still boom")
	...
	u := &GCSUploader{Inner: pp, Bucket: "b", MaxRetries: 2, BaseBackoff: time.Millisecond, Now: time.Now, Sleep: fastSleep}
...
func TestIsTransient_Classification(t *testing.T) {
	cases := []struct {
		...
		{"Unavailable is transient", status.Error(codes.Unavailable, ""), true},
		{"DeadlineExceeded is transient", status.Error(codes.DeadlineExceeded, ""), true},
		{"ResourceExhausted is transient", status.Error(codes.ResourceExhausted, ""), true},
		{"PermissionDenied is permanent", status.Error(codes.PermissionDenied, ""), false},
		{"NotFound is permanent", status.Error(codes.NotFound, ""), false},
		{"InvalidArgument is permanent", status.Error(codes.InvalidArgument, ""), false},
